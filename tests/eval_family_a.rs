...
fn inum_uses_the_active_inode_view() {
    ...
    .unwrap());
    assert!(evaluate(
        &RuntimeExpr::Predicate(RuntimePredicate::Inum(NumericComparison::Exactly(logical_inode))),
        &entry,
        FollowMode::Logical,
        &mut sink,
    )
...
fn links_uses_the_active_hard_link_count_view() {
    let root = tempdir().unwrap();
    fs::write(root.path().join("real.txt"), "hello\n").unwrap();
    fs::hard_link(root.path().join("real.txt"), root.path().join("real-hard.txt")).unwrap();
    unix_fs::symlink(root.path().join("real.txt"), root.path().join("file-link")).unwrap();
    ...
    .unwrap());
    assert!(evaluate(
        &RuntimeExpr::Predicate(RuntimePredicate::Links(NumericComparison::Exactly(logical_links))),
        &entry,
        FollowMode::Logical,
        &mut sink,
    )
